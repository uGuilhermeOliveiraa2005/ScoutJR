      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col gap-4">
          ...
          <Section icon={<User size={18} />} title="Dados da conta">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input defaultValue={profile.nome} className="w-full px-3 py-2.5 text-sm border border-neutral-200 rounded-lg bg-white outline-none focus:border-green-400" />
              </div>
              <div>
                <label className="block text-xs font-medium text-neutral-400 uppercase tracking-wide mb-1.5">E-mail</label>
                <input defaultValue={profile.email} disabled className="w-full px-3 py-2.5 text-sm border border-neutral-200 rounded-lg bg-neutral-50 text-neutral-400 cursor-not-allowed" />
              </div>
              {profile.telefone && (
                <div>
                  <label className="block text-xs font-medium text-neutral-400 uppercase tracking-wide mb-1.5">Telefone</label>
                  <input defaultValue={profile.telefone} className="w-full px-3 py-2.5 text-sm border border-neutral-200 rounded-lg bg-white outline-none focus:border-green-400" />
                </div>
              )}
            </div>
          ...
          <Section icon={<CreditCard size={18} />} title="Plano e pagamento">
            ...
            {isClube && (
              <>
                {clube?.status_assinatura === 'active' ? (
                  <StripeButton tipo="portal" label="Gerenciar assinatura no Stripe" variant="outline" />
                ) : (
                  <div className="grid grid-cols-3 gap-3">
                    {(['starter','pro','enterprise'] as const).map(p => (
                      <div key={p} className="border border-neutral-200 rounded-xl p-4 text-center">
                        <div className="font-display text-lg text-neutral-900 mb-1">{p.toUpperCase()}</div>
                        <div className="font-display text-2xl text-green-700 mb-3">
                          {p === 'starter' ? 'R$ 297' : p === 'pro' ? 'R$ 797' : 'R$ 1.497'}
                          <span className="text-xs text-neutral-400 font-sans font-normal">/mês</span>
                        </div>
                        <StripeButton tipo="assinatura" plano={p} label="Assinar" />
                      </div>
                    ))}
                  </div>
                )}
            ...
            {!isClube && (
              <StripeButton tipo="destaque" label="Ativar destaque por R$ 49/mês" variant="amber" />
            )}
          </Section>
          ...
          <Section icon={<Shield size={18} />} title="Segurança">
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between">
                ...
                <button className="px-4 py-2 text-sm border border-neutral-200 rounded-lg hover:bg-neutral-50 transition-colors">Alterar</button>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <button className="px-4 py-2 text-sm bg-green-700 text-white rounded-lg hover:bg-green-800 transition-colors">Ativar</button>
              </div>
            </div>
          </Section>
