...
export function AthleteForm({ initialData, athleteId, mode, onSubmit }: AthleteFormProps) {
  ...
  const [data, setData] = useState({
    ...
    conquistas: initialData?.atleta_conquistas?.map((c: any) => ({ titulo: c.titulo, ano: c.ano.toString(), descricao: c.descricao })) || [],
    ...
    <div className="w-full max-w-lg">
      ...
      <div className="mb-6">
        <div className="h-1.5 bg-neutral-200 rounded-full mb-3 overflow-hidden">
          <div className="h-full bg-green-400 rounded-full transition-all duration-500" style={{ width: `${(step / 6) * 100}%` }} />
        </div>
      ...
      <div className="mt-8 text-center pb-8">
        <Link href={mode === 'create' ? "/dashboard" : `/perfil/${athleteId}`} className="text-sm text-neutral-400 hover:text-neutral-600 flex items-center justify-center gap-1 transition-colors">
...
function StepPreview({ data, onBack, onNext }: any) {
  return (
    <div className="flex flex-col gap-6">
      <div className="mb-2">
      ...
      <div className="flex justify-between mt-4">
        <Button variant="outline" onClick={onBack}><ArrowLeft size={15} /> Voltar e Editar</Button>
        <Button variant="dark" onClick={onNext}>Tudo certo! Continuar <ArrowRight size={15} /></Button>
      </div>
    </div>
  )
}
